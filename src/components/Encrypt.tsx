...
export function Encrypt() {
  ...
  const [encryptionMode, setEncryptionMode] = useState<"myself" | "someone">(
    "myself",
  );
  ...
  const handleEncrypt = async () => {
    ...
    if (encryptionMode === "someone" && !recipientPublicKey.trim()) {
    ...
    try {
      ...
      if (encryptionMode === "myself") {
        // For self-encryption: use master key directly as AES key
        encryptionKey = keyResult.masterKey;
      } else {
        // For peer encryption: derive shared secret using X25519 ECDH
        ...
        const recipientPublicKeyBytes = new Uint8Array(
          atob(recipientPublicKey)
            .split("")
            .map((c) => c.charCodeAt(0)),
        );

        // Derive shared secret using ECDH
        encryptionKey = await x25519DeriveSharedSecret(
          ourPrivateKey,
          recipientPublicKeyBytes,
        );
      }

      // Import crypto functions for AES-GCM
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-4">
              <button
                onClick={() => setEncryptionMode("myself")}
                className={`px-4 py-2 rounded-md font-medium transition ${
                  encryptionMode === "myself"
              ...
              <button
                onClick={() => setEncryptionMode("someone")}
                className={`px-4 py-2 rounded-md font-medium transition ${
                  encryptionMode === "someone"
                    ? "bg-slate-900 text-white"
                    : "bg-slate-100 text-slate-700 hover:bg-slate-200"
                }`}
              >
          ...
          {/* Recipient Public Key Input (only for "someone" mode) */}
          {encryptionMode === "someone" && (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-3">
              ...
              <input
                ...
                placeholder="Enter recipient's X25519 public key (base64)"
              ...
              <p className="text-xs text-slate-500 mt-1">
                The recipient must share their public key with you
              </p>
            </div>
          )}
          ...
          <button
            ...
            disabled={
              !message.trim() ||
              isEncrypting ||
              (encryptionMode === "someone" && !recipientPublicKey.trim())
            }
